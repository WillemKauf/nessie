...
import static org.projectnessie.versioned.storage.dynamodb2.DynamoDB2Constants.TABLE_OBJS;
...
public final class DynamoDB2Backend implements Backend {
  private static final Logger LOGGER = LoggerFactory.getLogger(DynamoDB2Backend.class);
  ...
  public DynamoDB2Backend(@Nonnull DynamoDB2BackendConfig config, boolean closeClient) {
  ...
  public PersistFactory createFactory() {
    return new DynamoDB2PersistFactory(this);
  }
  ...
  public void eraseRepositories(Set<String> repositoryIds) {
    ...
    List<String> prefixed =
        repositoryIds.stream().map(DynamoDB2Backend::keyPrefix).collect(Collectors.toList());
