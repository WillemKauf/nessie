...
public class CatalogServiceImpl implements CatalogService {
  ...
  private IcebergStuff icebergStuff() {
    return new IcebergStuff(
      ...
      new EntitySnapshotTaskBehavior(
        backendExceptionMapper,
        serviceConfig.effectiveRetryAfterThrottled()
      ),
      executor
    );
  }
  ...
  public StorageUri locationForEntity(
    ...
    try {
      namespaces = contentService(apiContext).getMultipleContents(
        refName,
        hash,
        keysInOrder,
        false,
  ...
  public Stream<Supplier<CompletionStage<SnapshotResponse>>> retrieveSnapshots(
    ...
    GetMultipleContentsResponse contentResponse = contentService(
      apiContext
    ).getMultipleContents(
      reference.name(),
      reference.hashWithRelativeSpec(),
    ...
    return contentResponse
      ...
      .map(c -> {
        ...
        try {
          ...
          // view.`
          LOGGER.info(
            "Failed to retrieve snapshot ID for {}: {}",
            c.getContent(),
            e.toString()
          );
          return null;
        ...
        return (Supplier<CompletionStage<SnapshotResponse>>) () -> {
          ...
          return snapshotStage.thenApply(snapshot ->
            snapshotResponse(
              key,
              c.getContent(),
              reqParams,
  ...
  public CompletionStage<SnapshotResponse> retrieveSnapshot(
    ...
    ContentResponse contentResponse = contentService(apiContext).getContent(
      key,
      reference.name(),
      reference.hashWithRelativeSpec(),
      false,
    ...
    CompletionStage<NessieEntitySnapshot<?>> snapshotStage = icebergStuff()
      .retrieveIcebergSnapshot(snapshotId, content);

    return snapshotStage.thenApply(snapshot ->
      snapshotResponse(key, content, reqParams, snapshot, effectiveReference)
    );
  }
  ...
  private SnapshotResponse snapshotTableResponse(
    ...
    switch (reqParams.snapshotFormat()) {
      ...
      case ICEBERG_TABLE_METADATA:
        ...
        result = nessieTableSnapshotToIceberg(
          snapshot,
      ...
      default:
        throw new IllegalArgumentException(
          "Unknown format " + reqParams.snapshotFormat()
        );
    }
  ...
  private SnapshotResponse snapshotViewResponse(
    ...
    switch (reqParams.snapshotFormat()) {
      ...
      case ICEBERG_TABLE_METADATA:
        ...
        // TODO Response should respect the JsonView / spec-version
        // TODO Add a check that the original table format was Iceberg (not Delta)
        result = nessieViewSnapshotToIceberg(
          snapshot,
          optionalIcebergSpec(reqParams.reqVersion()),
          metadataPropertiesTweak(snapshot, effectiveReference)
        );
  ...
  CompletionStage<MultiTableUpdate> commit(
    ...
    List<ContentKey> allKeys = commit
      .getOperations()
      .stream()
      .map(CatalogOperation::getKey)
      .collect(toList());
    for (ContentKey key : allKeys) {
      requestMeta.addKeyAction(key, apiRequest);
    }

    GetMultipleContentsResponse contentsResponse = contentService(
      apiContext
    ).getMultipleContents(
      reference.name(),
      reference.hashWithRelativeSpec(),
    ...
    checkArgument(
      requireNonNull(contentsResponse.getEffectiveReference()) instanceof
      Branch,
    ...
    MultiTableUpdate multiTableUpdate = new MultiTableUpdate(
    ...
    CompletionStage<MultiTableUpdate> commitBuilderStage = completedStage(
      multiTableUpdate
    );
    ...
    for (CatalogOperation op : commit.getOperations()) {
      ...
      if (op.getType().equals(ICEBERG_TABLE)) {
        ...
        commitBuilderStage = applyIcebergTableCommitOperation(
          target,
          op,
          content,
          multiTableUpdate,
        ...
        verifyIcebergOperation(op, reference, content);
        commitBuilderStage = applyIcebergViewCommitOperation(
          target,
          op,
          content,
          multiTableUpdate,
        ...
        throw new IllegalArgumentException(
          "(Yet) unsupported entity type: " + op.getType()
        );
      }
    }
    ...
    multiTableUpdate
      .operations()
      .commitMeta(commitMetaBuilder.apply(message.toString()));
    ...
    return commitBuilderStage
      ...
      .thenCompose(updates -> {
        ...
        for (SingleTableUpdate tableUpdate : updates.tableUpdates()) {
          ...
          NessieId snapshotId = objIdToNessieId(
            snapshotObjIdForContent(content)
          );
          ...
          CompletionStage<NessieEntitySnapshot<?>> stage =
            icebergStuff.storeSnapshot(
              tableUpdate.snapshot.withId(snapshotId),
              content
            );
  ...
  public CompletionStage<Stream<SnapshotResponse>> commit(
    ...
    return commit(
      ...
      apiRequest,
      apiContext
    ).thenApply(updates -> // Finally, transform each MultiTableUpdate.SingleTableUpdate to a SnapshotResponse
      updates
        .tableUpdates()
        .stream()
        .map(singleTableUpdate ->
          snapshotResponse(
    ...
    if (content != null) {
      if (!op.getType().equals(content.getType())) {
        ...
        throw new NessieReferenceConflictException(
          referenceConflicts(
            conflict(Conflict.ConflictType.PAYLOAD_DIFFERS, op.getKey(), msg)
          ),
          msg,
          null
        );
  ...
  private CompletionStage<MultiTableUpdate> applyIcebergTableCommitOperation(
    ...
    if (content == null) {
      contentId = null;
      String icebergUuid = icebergOp.getSingleUpdateValue(
        AssignUUID.class,
        AssignUUID::uuid
      );
    ...
    CompletionStage<SingleTableUpdate> contentStage = snapshotStage
      .thenApply(nessieSnapshot -> {
        LOGGER.info(
      ...
      .thenApply(updateState -> {
        ...
        String metadataJsonLocation = icebergMetadataJsonLocation(
          nessieSnapshot.icebergLocation()
        );
        IcebergTableMetadata icebergMetadata = storeTableSnapshot(
          metadataJsonLocation,
          nessieSnapshot,
          multiTableUpdate
        );
        Content updated = icebergMetadataToContent(
          metadataJsonLocation,
          icebergMetadata,
          contentId
        );
        ...
        SingleTableUpdate singleTableUpdate = new SingleTableUpdate(
          nessieSnapshot,
          updated,
          icebergOp.getKey(),
    ...
    commitBuilderStage = contentStage.thenCombine(
      commitBuilderStage,
      (singleTableUpdate, nothing) -> multiTableUpdate
    );
    return commitBuilderStage;
  }
  ...
  private CompletionStage<MultiTableUpdate> applyIcebergViewCommitOperation(
    ...
    CompletionStage<SingleTableUpdate> contentStage = snapshotStage
      ...
      .thenApply(updateState -> {
        ...
        IcebergViewMetadata icebergMetadata = storeViewSnapshot(
          metadataJsonLocation,
          nessieSnapshot,
          multiTableUpdate
        );
  ...
  String setLocationForCreate(
    ...
    return op
      ...
      .map(l -> {
        ...
        validateStorageLocation(l).ifPresent(msg -> {
          throw new IllegalArgumentException(
            format(
      ...
      .orElseGet(() -> {
        ...
        WarehouseConfig w = lakehouseConfig
          .catalog()
          .getWarehouse(op.warehouse());
  ...
  @VisibleForTesting
  static List<IcebergMetadataUpdate> pruneCreateUpdates(
  ...
  private CompletionStage<NessieTableSnapshot> loadExistingTableSnapshot(
    Content content
  ) {
    ObjId snapshotId = snapshotObjIdForContent(content);
  ...
  private CompletionStage<NessieViewSnapshot> loadExistingViewSnapshot(
    Content content
  ) {
    ObjId snapshotId = snapshotObjIdForContent(content);
    return icebergStuff().retrieveIcebergSnapshot(snapshotId, content);
  ...
  private IcebergTableMetadata storeTableSnapshot(
    ...
    IcebergTableMetadata tableMetadata = nessieTableSnapshotToIceberg(
      snapshot,
      Optional.empty(),
      p -> {}
    );
  ...
  private <M> M storeSnapshot(
    ...
    try (
      OutputStream out = objectIO.writeObject(
        StorageUri.of(metadataJsonLocation)
      )
    ) {
      IcebergJson.objectMapper().writeValue(out, metadata);
    } catch (Exception ex) {
      throw new RuntimeException(
        "Failed to write snapshot to: " + metadataJsonLocation,
        ex
      );
    }
    return metadata;
  ...
  private static Optional<IcebergSpec> optionalIcebergSpec(
